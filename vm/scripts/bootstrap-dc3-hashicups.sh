#!/usr/bin/env bash
# Bootstrap a DC3 HashiCups VM (Consul client + Connect sidecar).
# Usage: sudo bash bootstrap-dc3-hashicups.sh <DC3_CONSUL_SERVER_IP>
#
# Before running, SCP from repo root:
#   scp -i $KEY -r vm/ ubuntu@$HC_IP:/tmp/
#   scp -i $KEY vm/consul_*.zip ubuntu@$HC_IP:/tmp/
#   ssh -i $KEY ubuntu@$HC_IP \
#     "sudo bash /tmp/vm/scripts/bootstrap-dc3-hashicups.sh $DC3_CONSUL_SERVER_IP"
#
# After this script, run the appropriate on-<service>.sh from vm/hashicups/scripts/.
set -euo pipefail

CONSUL_SERVER_IP="${1:?Usage: $0 <DC3_CONSUL_SERVER_IP>}"
...
NODE_NAME="dc3-$(hostname -s)"

echo "==> bootstrap-dc3-hashicups | node: ${NODE_NAME} | joining: ${CONSUL_SERVER_IP}"
...
sed -e "s/REPLACE_WITH_PRIVATE_IP/${PRIVATE_IP}/g" \
    ...
    "${VM_DIR}/consul/dc3-client.hcl" > /etc/consul.d/client.hcl
...
# --- Envoy sidecar unit template ---
cp "${VM_DIR}/hashicups/systemd/connect-envoy@.service" /etc/systemd/system/
systemctl daemon-reload

echo "==> bootstrap-dc3-hashicups complete | node: ${NODE_NAME}"
echo "    Now run the service script: sudo bash /tmp/vm/hashicups/scripts/on-<role>.sh"
