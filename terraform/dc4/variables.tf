...
variable "vpc_cidr" {
  ...
  description = "CIDR for the DC4 VPC. Must not overlap with DC3 (10.10.0.0/16)."
}
...
# Same service set as DC3. ESM will health-check these external VMs.
...
variable "peer_connection_id" {
  type        = string
  default     = ""
  description = "VPC peering connection ID from dc3. Set after applying dc3 to accept peering."
}

variable "peer_vpc_cidr" {
  type        = string
  default     = "10.10.0.0/16"
  description = "DC3 VPC CIDR. Used for routing and security group rules."
}
...
variable "enable_k8s_partition" {
  type        = bool
  default     = false
  description = "Open Consul agent ports (8300-8503 TCP, 8301-8302 UDP) from the VPC CIDR so DC6 EKS worker nodes can join the Consul cluster as the dc6-k8s admin partition."
}
